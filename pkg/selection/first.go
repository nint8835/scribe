...
func firstQuoteLeastSeen(ctx context.Context, userId string, tiebreaker TiebreakerMethod) (database.Quote, error) {
...
func firstQuoteLeastSeenGlobal(ctx context.Context, userId string, tiebreaker TiebreakerMethod) (database.Quote, error) {
	var quote database.Quote
	db := database.Instance.WithContext(ctx)
...
func firstQuoteClosestElo(ctx context.Context, userId string, tiebreaker TiebreakerMethod) (database.Quote, error) {
	var quote database.Quote
	db := database.Instance.WithContext(ctx)
...
func firstQuoteLastWinner(ctx context.Context, userId string, _ TiebreakerMethod) (database.Quote, error) {
	var quote database.Quote
	db := database.Instance.WithContext(ctx)
...
func firstQuoteLastLoser(ctx context.Context, userId string, _ TiebreakerMethod) (database.Quote, error) {
	var quote database.Quote
	db := database.Instance.WithContext(ctx)
	...
	err := db.Raw(
		...
		FROM
			completed_comparisons
			JOIN quotes ON quotes.id = (
				CASE
					WHEN completed_comparisons.winner_id = completed_comparisons.quote_a_id THEN completed_comparisons.quote_b_id
					ELSE completed_comparisons.quote_a_id
				END
			)
		WHERE
			completed_comparisons.user_id = ?
...
func firstQuoteRandom(ctx context.Context, _ string, _ TiebreakerMethod) (database.Quote, error) {
	var quote database.Quote
	db := database.Instance.WithContext(ctx)
...
const (
	...
	FirstQuoteMethodLastLoser       FirstQuoteMethod = "last_loser"
...
var firstQuoteSelectors = map[FirstQuoteMethod]FirstQuoteSelector{
	...
	FirstQuoteMethodLastLoser:       firstQuoteLastLoser,
	FirstQuoteMethodRandom:          firstQuoteRandom,
}

var DefaultFirstQuoteMethod = FirstQuoteMethodLeastSeen

func selectFirstQuote(ctx context.Context, userId string, method FirstQuoteMethod, tiebreaker TiebreakerMethod) (database.Quote, error) {
	selector, ok := firstQuoteSelectors[method]
	if !ok {
		selector = firstQuoteSelectors[DefaultFirstQuoteMethod]
	}
	return selector(ctx, userId, tiebreaker)
}
